...
\cs(255, 255, 64)${key_bind_movespeed_lock}\cs(200, 200, 200)Jute Boots:\cr ${toggle_movespeed_lock}
]]
...
hub_options_lte = [[ \cs(255, 115, 0)== Options: \cr              \cs(255, 255, 64)${key_bind_matchsc}\cs(200, 200, 200)Match SC Element:\cr ${player_match_sc}            \cs(255, 255, 64)${key_bind_lock_weapon} \cs(200, 200, 200)Lock Weapon:\cr ${toggle_lock_weapon}            \cs(255, 255, 64)${key_bind_movespeed_lock}\cs(200, 200, 200)Jute Boots:\cr ${toggle_movespeed_lock} ]]
...
function update_active_ja(name, gain)
	...
	Buff['Feint'] = buffactive['Feint'] or false
...
function Set_TH_Gear()
	if buffactive['Sneak attack'] then
		Equip = {Hands = "Plun. Armlets +1",Feet = "Skulk. Poulaines +1"}
	end
...
function updateRunspeedGear( value )   
    ...
    if not value then
        if use_UI == true then
            ...
            windower.add_to_chat(8,"----- Locking Off Jute Boots +1 -----")   
        end
        enable('Feet')
        idle()
    else
        if use_UI == true then
            ...
            windower.add_to_chat(8,"----- Locking On Jute Boots +1 -----")
        end
